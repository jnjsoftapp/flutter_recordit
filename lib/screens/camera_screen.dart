  ...
  Future<void> _addExifData(String imagePath, Position position) async {
    try {
      ...
      if (tags != null) {
        ...
        // EXIF 데이터 직접 수정
        tags['GPS GPSLatitudeRef'] = position.latitude >= 0 ? 'N' : 'S';
        tags['GPS GPSLatitude'] = '$latDeg/1,$latMin/1,$latSec/1000';
        tags['GPS GPSLongitudeRef'] = position.longitude >= 0 ? 'E' : 'W';
        tags['GPS GPSLongitude'] = '$longDeg/1,$longMin/1,$longSec/1000';
        
        // 현재 시간 추가
        final now = DateTime.now();
        final dateTimeStr = '${now.year}:${now.month.toString().padLeft(2, '0')}:${now.day.toString().padLeft(2, '0')} '
            '${now.hour.toString().padLeft(2, '0')}:${now.minute.toString().padLeft(2, '0')}:${now.second.toString().padLeft(2, '0')}';
        
        tags['DateTime'] = dateTimeStr;

        // 수정된 EXIF 데이터를 파일에 저장
        await writeExifToFile(tags, imagePath);
      }
    } catch (e) {
